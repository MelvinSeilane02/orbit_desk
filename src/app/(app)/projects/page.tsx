...
import { getProjectsForList, getAttentionItems } from "@/lib/data";
...
import { OfflineProjectsPage } from "./OfflineProjectsPage";

export const metadata: Metadata = { title: "Projects — Orbit Desk" };
...
export default async function ProjectsPage({
  ...
  searchParams: Promise<{ tab?: string; view?: string; q?: string; client?: string }>;
}) {
  if (isOfflineMode()) return <OfflineProjectsPage />;

  const { tab = "active", view = "list", q = "", client: clientFilter = "" } = await searchParams;
  const workspace = await requireWorkspace();
  const [allRows, attention, clients] = await Promise.all([
    getProjectsForList(workspace.id),
      ...
      <div className="flex flex-none flex-col gap-4 px-[30px] pt-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-[10px]">
            <Link href="/projects?new=1" className="od-btn od-btn-p">
              New project
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-[10px]">
          <form method="GET" className="flex items-center gap-[10px]">
...
type Row = Awaited<ReturnType<typeof getProjectsForList>>[number];
          ...
          return (
            <Link key={p.id} href={`/projects/${p.id}`} className={flagged ? "od-drawer od-drawer-flag" : "od-drawer"} style={{ cursor: "pointer" }}>
              ...
              <span className="od-num text-right text-[13.5px]" style={{ width: 100 }}>
                {formatMoney(p.fixedPrice.toString(), currency)}
              </span>
...
function ByClientView({ rows, currency }: { rows: Row[]; currency: string }) {
  const groups = new Map<string, { client: string; clientId: string; items: Row[] }>();
  for (const p of rows) {
  ...
  return (
    <div className="flex min-h-0 flex-col gap-5">
      ...
      {list.map((g) => {
        const booked = g.items.reduce((s, p) => s + Number(p.fixedPrice.toString()), 0);
        return (
