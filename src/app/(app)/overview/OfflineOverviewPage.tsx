...
import { useOverviewCounts, useAttentionItems } from "@/lib/offline/reads";
import { archiveProject } from "@/lib/offline/writes/projects";
import { AttentionRow } from "@/components/projects/AttentionRow";

export function OfflineOverviewPage() {
  const { user, workspace } = useOfflineWorkspace();
  const counts = useOverviewCounts(workspace.id);
          ...
          <div className="grid grid-cols-1 gap-0 md:grid-cols-2 md:gap-x-[14px]">
            {attention.map((a) => (
              <AttentionRow
                key={a.projectId}
                item={a}
                archiveAction={(fd) => archiveProject(workspace.id, fd)}
              />
            ))}
          </div>
        </div>
