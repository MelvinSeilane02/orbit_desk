...
import { useArchive } from "@/lib/offline/reads";
import { restoreProject } from "@/lib/offline/writes/projects";
...
export function OfflineArchivePage() {
  ...
  const tab = searchParams.get("tab") ?? "rejected";
  const { rejected, archived } = useArchive(workspace.id);
          ...
          {rows.map((p) =>
            tab === "rejected" ? (
              ...
              <div key={p.id} className="od-drawer od-drawer-closed" style={{ padding: "12px 16px" }}>
                ...
                <form
                  action={async (fd) => {
                    await restoreProject(workspace.id, fd);
