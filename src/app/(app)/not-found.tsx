...
import { OfflineNotFound } from "./OfflineNotFound";

export default async function AppNotFound() {
  if (isOfflineMode()) return <OfflineNotFound />;

  const workspace = await requireWorkspace();
  const [projectCount, clientCount, archiveCount] = await Promise.all([
    prisma.project.count({ where: { workspaceId: workspace.id, stage: { not: "rejected" }, archivedAt: null } }),
    prisma.client.count({ where: { workspaceId: workspace.id } }),
    prisma.project.count({ where: { workspaceId: workspace.id, OR: [{ stage: "rejected" }, { archivedAt: { not: null } }] } }),
  ]);
