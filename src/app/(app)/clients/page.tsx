...
import { getClientsForList, daysAgo } from "@/lib/data";
...
import { OfflineClientsPage } from "./OfflineClientsPage";

export const metadata: Metadata = { title: "Clients — Orbit Desk" };
...
export default async function ClientsPage({
  ...
  searchParams: Promise<{ tab?: string; view?: string; q?: string }>;
}) {
  if (isOfflineMode()) return <OfflineClientsPage />;

  const { tab = "all", view = "all", q = "" } = await searchParams;
  const workspace = await requireWorkspace();
  const clients = await getClientsForList(workspace.id);
      ...
      <div className="flex flex-col gap-4 px-[30px] pt-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-[10px]">
            <Link href="/clients?new=1" className="od-btn od-btn-p">
              New client
            </Link>
          </div>
        </div>
...
type Row = Awaited<ReturnType<typeof getClientsForList>>[number];

function AllView({ clients }: { clients: Row[] }) {
  return (
    <>
...
function LastContactView({ clients }: { clients: Row[] }) {
...
function ClientRow({ client: c }: { client: Row }) {
  return (
